...
def build_action_register(kpis: list[dict[str, object]], cc_rows: list[dict[str, object]], vendor_rows: list[dict[str, object]], reorder: list[dict[str, object]], channel_rows: list[dict[str, object]], forecast_best: dict[str, object]) -> list[dict[str, object]]:
    ...
    return [
        {"action_id": "ACT-001", "module": "FI", "issue": "Overdue receivables", "evidence": f"Open AR balance {float(kpi_lookup['Open AR balance']):,.2f}", "priority": "High", "recommended_action": "Prioritize 90+ and 61-90 day receivables for collection follow-up.", "owner_role": "Finance / Credit Control", "expected_business_value": "Improve cash conversion and reduce write-off exposure."},
        ...
        {"action_id": "ACT-003", "module": "MM", "issue": "Inventory item below reorder point", "evidence": f"{len(reorder)} reorder alerts generated", "priority": "High", "recommended_action": "Replenish below-threshold items and confirm reorder points for seasonal demand.", "owner_role": "Inventory Controller", "expected_business_value": "Reduce stockout risk in housekeeping, F&B, and maintenance operations."},
